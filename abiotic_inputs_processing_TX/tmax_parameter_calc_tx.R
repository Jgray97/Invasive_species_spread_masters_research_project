# CALCULATING TEMPERATURE RF PARAMETER VALUES FOR GRID CELLS ACROSS TEXAS
# Author = John Gray
...
# Load in establishment tracker to provide list of Texas grid cells
establishment_tracker <- read.csv("data/TX_egoose_establishment_ct-50_met-0.005.csv")

# Filter temp dataframe to just include Texas cells
tmax_2000_texas <- filter(tmax_2000_df, tmax_2000_df$cell %in% establishment_tracker$grid_cell)

# change col names
colnames(tmax_2000_texas) <- c("lon", "lat", "tmax", "cell")

# calculate average temperature value for each cell in Texas
tmax_cells_2000 <- tmax_2000_texas %>%
  group_by(cell) %>%
...
for (i in 2001:2022) {
  ...
  # Filter to just include entries belonging to cells in Texas
  tmax_texas <- filter(tmax_df, tmax_df$cell %in% establishment_tracker$grid_cell)
  
  # Update column names
  colnames(tmax_texas) <- c("lon", "lat", "tmax", "cell")
  
  # Calculate average values for each Texas cell and store in a dataframe
  tmax_cells <- tmax_texas %>%
    group_by(cell) %>%
    summarise(mean_tmax = mean(tmax))

...
encounter_rates_00_05 <- read.csv("data/TX_encounter_rates_2000-2005.csv")
...
encounter_rates <- rbind(encounter_rates_00_05, encounter_rates_06_08, 
...
write.csv(common_birds, "data/texas_common_bird_encounter_rates_2000_2015.csv")
...
write.csv(common_native_birds, "data/texas_common_native_bird_encounter_rates_2000_2015.csv")
...
for (i in 1:length(texas_cells)) {
  ...
  n_plus_83 <- filter(common_native_birds, cell == (texas_cells[i]+83))
  
  n_minus_80 <- filter(common_native_birds, cell == (texas_cells[i]-83))
  
  n_minus_83 <- filter(common_native_birds, cell == (texas_cells[i]-83))
  
  n_plus_162 <- filter(common_native_birds, cell == (texas_cells[i]+162))
  
  n_plus_163 <- filter(common_native_birds, cell == (texas_cells[i]+163))
  
  n_plus_164 <- filter(common_native_birds, cell == (texas_cells[i]+164))
  
  n_minus_162 <- filter(common_native_birds, cell == (texas_cells[i]-162))
  
  n_minus_163 <- filter(common_native_birds, cell == (texas_cells[i]-163))
  
  n_minus_164 <- filter(common_native_birds, cell == (texas_cells[i]-164))
  ...
  for (j in 2003:2019) {
    rf_biotic_data$bs_ring_2[(rf_biotic_data$year == j) & 
      ...
      (if((texas_cells[i] + 83) %in% texas_cells) {
        (establishment_tracker$establishment_score[(establishment_tracker$year == (j-1)) & 
                                                     (establishment_tracker$grid_cell == (texas_cells[i]+83))] * n_plus_83_similarity)
      } else {
        0
      }) +
      (if((texas_cells[i] - 80) %in% texas_cells) {
      ...
      (if((texas_cells[i] - 83) %in% texas_cells) {
        (establishment_tracker$establishment_score[(establishment_tracker$year == (j-1)) & 
                                                     (establishment_tracker$grid_cell == (texas_cells[i]-83))] * n_minus_83_similarity)
      } else {
        0
        }) +
      ...
      (if((texas_cells[i] + 163) %in% texas_cells) {
        (establishment_tracker$establishment_score[(establishment_tracker$year == (j-1)) & 
                                                     (establishment_tracker$grid_cell == (texas_cells[i]+163))] * n_plus_163_similarity)
      } else {
        0
      }) +
      (if((texas_cells[i] + 164) %in% texas_cells) {
        (establishment_tracker$establishment_score[(establishment_tracker$year == (j-1)) & 
                                                     (establishment_tracker$grid_cell == (texas_cells[i]+164))] * n_plus_164_similarity)
      } else {
        0
      }) +
      (if((texas_cells[i] - 162) %in% texas_cells) {
      ...
      (if((texas_cells[i] - 163) %in% texas_cells) {
        (establishment_tracker$establishment_score[(establishment_tracker$year == (j-1)) & 
                                                     (establishment_tracker$grid_cell == (texas_cells[i]-163))] * n_minus_163_similarity)
      } else {
        0
      }) +
      (if((texas_cells[i] - 164) %in% texas_cells) {
        (establishment_tracker$establishment_score[(establishment_tracker$year == (j-1)) & 
                                                     (establishment_tracker$grid_cell == (texas_cells[i]-164))] * n_minus_164_similarity)
      } else {
        0
      })
  }
